...
class StoreModuleSite extends WeModuleSite {
			...
			if (0 == $goods['status']) {
				...
				if (!empty($module_expired_list)) {
					...
					if ($expired_modules) {
						...
						itoast('应用：' . $expired_modules_name . '，服务费到期，无法上架！', referer(), 'error');
					}
				}
			}
	...
	public function doWebGoodsPost() {
		...
		if ('post' == $operate) {
			...
			if (checksubmit('submit')) {
				...
				if (STORE_TYPE_PACKAGE == $type) {
					$group_modules = module_name_list($data['module_group']);
					$module_name = empty($module_name) ? $group_modules : array_merge($module_name, $group_modules);
				}
				$module_expired_list = module_expired_list();
				if (is_error($module_expired_list)) {
					itoast($module_expired_list['message'], '', '');
				...
				if (!empty($module_expired_list)) {
					...
					if ($expired_modules) {
						$expired_modules = pdo_getall('modules', array('name IN' => $expired_modules), array('title'), 'title');
						$expired_modules_name = implode('，', array_column($expired_modules, 'title'));
						itoast('应用：' . $expired_modules_name . '，服务费到期，无法添加！', referer(), 'error');
					}
				}
	...
	public function doWebGoodsBuyer() {
		...
		if ('submit_order' == $operate) {
			...
			$module_name = array();
			if (in_array($goods_info['type'], array(STORE_TYPE_MODULE, STORE_TYPE_WXAPP_MODULE, STORE_TYPE_WEBAPP_MODULE, STORE_TYPE_PHONEAPP_MODULE, STORE_TYPE_ALIAPP_MODULE, STORE_TYPE_BAIDUAPP_MODULE, STORE_TYPE_TOUTIAOAPP_MODULE))) {
				$module_name = array($goods_info['module']);
			}
			if (STORE_TYPE_PACKAGE == $goods_info['type']) {
				$module_name = module_name_list($goods_info['module_group']);
			}
			if (STORE_TYPE_USER_PACKAGE == $goods_info['type']) {
				$user_group = pdo_get('users_group', array('id' => $goods_info['user_group']), array('package'));
				$moudels_group = iunserializer($user_group['package']);
				$module_name = module_name_list($moudels_group);
			}
			...
			if (!empty($module_name)) {
				...
				if (!empty($module_expired_list)) {
					...
					if ($expired_modules) {
						$expired_modules = pdo_getall('modules', array('name IN' => $expired_modules), array('title'), 'title');
						$expired_modules_name = implode('，', array_column($expired_modules, 'title'));
						iajax(-1, '应用：' . $expired_modules_name . '，服务费到期，请联系管理员！');
					}
				}
			}
